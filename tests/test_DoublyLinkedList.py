...
def test_insert_remove(empty_dll):
    ...
    first = empty_dll.insert_between(1, empty_dll._header, empty_dll._trailer)
    second = empty_dll.insert_between(3, first, empty_dll._trailer)
    third = empty_dll.insert_between(5, second, empty_dll._trailer)
    assert len(empty_dll) == 3
    assert empty_dll.delete_node(second) == 3
    assert len(empty_dll) == 2
    assert empty_dll.delete_node(third) == 5
    empty_dll.delete_node(first)
    assert len(empty_dll) == 0
    assert empty_dll._header._next == empty_dll._trailer
    assert empty_dll._trailer._prev == empty_dll._header
